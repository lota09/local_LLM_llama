...
2. **Vulkan 백엔드는 "양자화 KV 캐시 + 일정 크기 이상 컨텍스트" 조합에서 성능이 붕괴한다.** 채운 토큰을 175개로 고정한 통제 실험에서, `-ctk/-ctv q4_0`으로 컨텍스트를 20,480 이상 할당하면 토큰 생성이 69.9 → 8.9 t/s로 87% 하락한다. **그러나 컨텍스트를 그대로 두고 KV 타입만 f16으로 바꾸면 71.3 t/s로 완전히 회복된다.** 즉 컨텍스트 크기 자체가 아니라 양자화 KV와의 상호작용이 원인이다. 동일 조건에서 ROCm/HIP는 q4_0으로도 전 구간 평탄하다(59~60 t/s).
...
| **전력 상한** | 178.0 W |
| PCIe 링크 | 8.0 GT/s ×4 (PCIe 3.0 x4 상당) |
...
| 사용 조건 | 권장 구성 | 근거 |
|:---|:---|:---|
| 생성 속도 최우선 | **Vulkan + f16 KV** | tg 71.3 t/s (전 구간 최고) |
| 긴 프롬프트/문서 처리 위주 | **ROCm/HIP** | pp 903 vs 454 t/s (1.99배) |
| VRAM 절약이 꼭 필요한 초장문 컨텍스트 | **ROCm/HIP + q4_0 KV** | HIP는 q4_0으로도 평탄, f16 대비 KV 3.6배 절약 |
| 설치 간편성 우선 | **Vulkan + f16 KV** | Mesa만으로 동작, ROCm 수 GB 불필요 |
| **피해야 할 조합** | ~~Vulkan + q4_0 KV (ctx ≥ 20K)~~ | 8.89 t/s로 붕괴, 이득 없음 |
...
2. **Vulkan에서 양자화 KV 캐시(q4_0)는 일정 컨텍스트 이상에서 사용 불가 수준으로 느려진다.** 채운 토큰을 고정한 통제 실험에서 임계값은 16,384~20,480 사이였고, 그 이상에서는 컨텍스트 크기와 무관하게 일정한 8.89 t/s를 보였다. **KV 타입을 f16으로 바꾸는 것만으로 71 t/s로 회복**되며, 이는 q4_0의 최고 성능(69.9)보다도 빠르다. 따라서 Vulkan 백엔드에서 양자화 KV는 VRAM을 아끼는 대가로 성능을 잃는 선택이 아니라, **아무 이득 없이 성능만 잃는 선택**이다.
