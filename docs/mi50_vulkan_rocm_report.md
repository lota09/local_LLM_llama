...
2. **Vulkan 백엔드는 할당 컨텍스트 32,768 이상에서 성능이 붕괴한다.** 실제로 채운 토큰이 225개로 동일함에도, KV 캐시를 크게 "할당"하는 것만으로 토큰 생성이 69.7 → 8.9 t/s로 87% 하락한다. 동일 조건에서 ROCm/HIP는 59.5 t/s로 평탄하다. 이는 하드웨어 한계가 아니라 백엔드 구현 특성이다.
...
- 175,126에서 32,768보다 오히려 빠른(17.4 vs 8.9) 비단조성이 관찰된다. 단순한 "클수록 느림" 모델로는 설명되지 않으며, 특정 크기 구간에서 커널 선택 경로가 달라지는 것으로 추정된다.
- ROCm/HIP는 4,096 ~ 175,126 전 구간에서 57.8 ~ 60.0 t/s로 **완전히 평탄**하다. 편차 4% 이내.
- 채운 토큰이 동일하므로 이는 **연산량 증가가 아닌 할당 크기에 기인**한다.
...
| 사용 조건 | 권장 백엔드 | 근거 |
|:---|:---|:---|
| 컨텍스트 ≤ 16K, 생성 위주 | **Vulkan** | tg 69.7 vs 57.8 t/s (1.21배) |
| 컨텍스트 ≥ 32K | **ROCm/HIP** | tg 59.5 vs 8.9 t/s (6.7배) |
| 긴 프롬프트/문서 처리 위주 | **ROCm/HIP** | pp 903 vs 454 t/s (1.99배) |
| 설치 간편성 우선 | **Vulkan** | Mesa만으로 동작, ROCm 수 GB 불필요 |
| 안정성/예측가능성 우선 | **ROCm/HIP** | 전 컨텍스트 구간 평탄 |
...
- Vulkan 성능 붕괴의 근본 원인을 소스 수준에서 규명하지 않았다. 32,768 임계값과 175,126에서의 비단조성은 커널 선택 로직 분석이 추가로 필요하다.
...
2. **"할당한" 컨텍스트 크기가 Vulkan 백엔드에서 실제 사용량과 무관하게 성능을 결정한다.** 채운 토큰 225개를 고정한 통제 실험에서 32,768 할당만으로 87% 성능 저하가 재현되었다. 이는 VRAM 용량이 큰 카드에서 컨텍스트를 자동 산출하는 스크립트가 오히려 성능을 해칠 수 있음을 의미한다.
